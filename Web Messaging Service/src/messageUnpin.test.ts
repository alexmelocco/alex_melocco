...
describe('Test for message/pin/v1', () => {
  test('Deals with invalid token', () => {
    ...
    sendWrapRequest('POST', '/message/pin/v1', { token: user1.token }, {
      messageId: message.messageId,
    });
    const data = sendWrapRequest('POST', '/message/unpin/v1', { token: (user1 + user2).token }, {
      messageId: message.messageId,
    });
  ...
  test('Member is not a part of the channel', () => {
    ...
    const data = sendWrapRequest('POST', '/message/unpin/v1', { token: user2.token }, {
      messageId: message.messageId,
    });
  ...
  test('Deals with invalid messageId within a channel or DM that the user is part of', () => {
    ...
    const data = sendWrapRequest('POST', '/message/unpin/v1', { token: user1.token }, {
  ...
  test('Deals with the message is already not pinned', () => {
    const user1 = sendWrapRequest('POST', '/auth/register/v3', {},
      { email: 'personone@email.com', password: 'passwordone', nameFirst: 'Person', nameLast: 'One' });
    ...
    const data2 = sendWrapRequest('POST', '/message/unpin/v1', { token: user1.token }, {
      messageId: message.messageId,
    });
  ...
  test('Deals with user is a global owner', () => {
    ...
    sendWrapRequest('POST', '/message/pin/v1', { token: user2.token }, {
